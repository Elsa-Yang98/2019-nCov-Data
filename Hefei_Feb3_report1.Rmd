...
```
This report on Hefei almost exactly follows the approach used in the 2nd preliminary analysis on international data [second preliminary analysis](https://htmlpreview.github.io/?https://github.com/qingyuanzhao/2019-nCov-Data/blob/master/Feb3.html)
...
```{r}
source("functions.R")
data <- read.table("Hefei_Feb3.tsv", sep = "\t", header = TRUE)
data=data[-32,]
...
fit <- lm(log(OI_mean / (N - date + 1)) ~ date, subset(df, date > 25 & date < 50))
#fit <- lm(log(OI_mean / (N - date + 1)) ~ date, df)
